...
class ProductController extends Controller
{
    ...
    public function show(Request $request, Product $product)
    {
        ...
        // URL konten disembunyikan dari user biasa (lihat $hidden di model),
        // tapi admin perlu melihatnya untuk mengedit di form. Buka khusus admin.
        ...
        // Rata-rata rating dihitung dari seluruh review user (bukan kolom statis).
     ...
     *
     * Konten belajar produk dalam bentuk seragam (sections → items) untuk
     * ketiga tipe (modul/kelas/bootcamp). URL konten asli (video/pdf/live)
     * HANYA dikeluarkan untuk:
     *   - user yang sudah membeli produk ini (transaksi paid), atau
     *   - item yang ditandai is_free (preview gratis).
     * Selain itu item dikunci (`locked: true`, `content_url: null`).
     *
     * Endpoint ini ter-gate auth:sanctum. Halaman jualan publik tetap pakai
     * GET /products/{id} yang tidak pernah mengembalikan URL konten.
     */
    public function content(Request $request, Product $product)
    {
        ...
        // Tiap item: buka URL hanya bila sudah beli atau item gratis.
        ...
        // Bootcamp: sertakan jadwal batch (info publik, tidak ter-gate).
     ...
     * Hanya relasi yang key-nya dikirim yang diganti (replace-all),
     * sehingga PATCH parsial tetap aman.
     */
    private function syncNested(Product $product, Request $request): void
    {
        ...
        if ($request->has('curriculum_sections')) {
            // Hapus section lama; FK cascade ikut menghapus items.
    ...
    /**
     * Validasi payload create/update. Saat update ($id terisi), field utama
     * boleh "sometimes" agar dukung PATCH parsial.
     */
    private function validateData(Request $request, ?int $id = null): array
    {
        ...
        return $request->validate([
            ...
            // curriculum_sections + items (kelas / bootcamp)
